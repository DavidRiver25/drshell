...
pub fn display_previous_cmd() {
    ...
    if status.in_history_mode && status.last_direction == Direction::Down {
...
pub fn display_next_cmd() {
    ...
    if status.in_history_mode && status.last_direction == Direction::Up {
...
pub fn input_history() -> Option<String> {
    let history = HISTORY_CMDS.lock().unwrap();
    let status = HISTORY_STATUS.lock().unwrap();
